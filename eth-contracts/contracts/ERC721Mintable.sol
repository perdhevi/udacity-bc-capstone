...
contract Ownable {
    ...
    modifier onlyOwner() {
        require(msg.sender == _owner, "only owner allowed to call");
        _;
    }
    ...
    event OwnershipTransfer(address fromAddress, address toAddress);
...
contract ERC721 is Pausable, ERC165 {
    ...
    function approve(address to, uint256 tokenId) public {
        ...
        require(to != owner, "Must not be token owner");
...
contract ERC721Metadata is ERC721Enumerable, usingOraclize {
    ...
    function _setTokenURI(uint256 tokenId) internal {
        ...
        string memory strToken = usingOraclize.uint2str(tokenId);
        _tokenURIs[tokenId] = usingOraclize.strConcat(_baseTokenURI, strToken);
    }
}
...
//      -takes in a 'to' address, tokenId, and tokenURI as parameters <-- no need tokenURI as it is calculated from baseToken
