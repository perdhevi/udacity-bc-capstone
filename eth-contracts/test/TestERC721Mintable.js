...
contract('TestERC721Mintable', accounts => {
    ...
    describe('match erc721 spec', function () {
        beforeEach(async function () { 
            this.contract = await ERC721Mintable.new("Udacity Property Coin", "UPC",{from: account_one} );
        ...
        it('should return total supply', async function () { 
            ...
            assert.equal(supply == 10, true, "Incorrect supply ".concat(supply.toString()) );
        })

        it('should get token balance', async function () { 
            let balance = await this.contract.balanceOf.call(account_one, {from: account_one});
            assert.equal(balance == 10, true, "Incorrect balance");
        })
        ...
        it('should transfer token from one owner to another', async function () { 
            ...
            assert.equal(owner == account_two, true, 'transfer failed : '.concat(owner));
        })
    });
    ...
    describe('have ownership properties', function () {
        ...
        it('should fail when minting when address is not contract owner', async function () { 
            ...
            assert.equal(owner == account_two, false, "Only contract owner allowed to mint");
        })

        it('should return contract owner', async function () { 
            let owner = await this.contract.owner.call();

            assert.equal(owner == account_one, true, "Owner is account one");
        })
